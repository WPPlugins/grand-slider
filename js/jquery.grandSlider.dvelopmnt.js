...
/*
 *  Grand Animation ver 1.0 - jQuery plugin 
 *  Written by zourbuth.com. 
 *
 *	 Dual licensed under the MIT
 *	 and GPL licenses.
 *
...
(function($){ 
    $.fn.extend({ 
        onwerDocument: function() { 
			//$(this).OwnerDocuments();

			var htmlStr = $('.grand-captions', this).html();

			$('.grand-caption', this).html(htmlStr);

			$(".grand-caption div", this).each(function(i) {
				$(this).css('display', 'block');
				$(this).animate({ opacity: 0 }, 0);
				var moving = $(this).attr('class');
				var easing = $(this).attr('slide-easing');
				var distance = $(this).attr('slide-distance');
				var speed = $(this).attr('slide-speed');
				...
				if ( moving == "slide-top" ) {
					properties = "top";
					sTop = position.top + parseInt( distance );
				}
				else if ( moving == "slide-bottom" ) {
					properties = "top";
					sTop = position.top - parseInt( distance );
				}
				else if ( moving == "slide-left" ) {
					properties = "left";
					sTop = position.left + parseInt( distance );
				}
				else if ( moving == "slide-right" ) {
					properties = "left";
					sTop = position.left - parseInt( distance );
				}
				
				var anim = {opacity: 1};
				anim[properties] = sTop+"px";

				$(this).delay(400*i).animate( anim, speed, easing );
			});
